...
**Status 2026-09-22 (evening):** live. Code deployed (`8138048`), studio tables on the production Postgres, Supabase S3 access key *"MLP Educator Studio (Netlify production)"* set on Netlify as `STUDIO_S3_ACCESS_KEY_ID` / `STUDIO_S3_SECRET_ACCESS_KEY` (production context, secret) and redeployed; bucket CORS allows `PUT` from any origin; a signed upload + direct PUT from the site was verified. The Youth Africa bundle was imported (`--set-main`): 772 assets / 797 files, 25 videos, 25 templates, 430 segments, 257 media matches — `/api/studio/library/playlists` on the site lists it as the main playlist with 25/25 ready. Still empty on Netlify: `OPENAI_API_KEY` (translation) and `ELEVENLABS_API_KEY` (AI voice); no render worker is running yet, so **Review & Generate queues jobs that nothing picks up** until a worker (section 4) is started against the production database and bucket.
