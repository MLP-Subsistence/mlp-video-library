...
**Workspace panels fold (2026-09-22):** on desktop the Segments column, the Script & Narration column and the visual preview each collapse (`panels` state in `workspace.tsx`; folded side columns become 44 px rails with a vertical label that reopen them, `GRID_COLUMNS` holds the Tailwind literals). With the preview folded the timeline gets the middle column and `large` tracks (112 px, big thumbnails). Mobile layout is unchanged.
